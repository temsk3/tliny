...
// コメント作成
export const v2_sns_comment_createComment = onCall(async (request: { data: { postId: string; content: string }; auth?: { uid?: string } }) => {
...
// コメント一覧取得
export const v2_sns_comment_getComments = onCall(async (request: { data: { postId: string; limit?: number; lastCommentId?: string }; auth?: { uid?: string } }) => {
  ...
  try {
    ...
    // ページネーション対応
    if (lastCommentId) {
      const lastCommentDoc = await db.collection('v/1/post_comments').doc(lastCommentId).get()
...
// コメント削除
export const v2_sns_comment_deleteComment = onCall(async (request: { data: { commentId: string }; auth?: { uid?: string } }) => {
  ...
  try {
    ...
    const commentDoc = await db.collection('v/1/post_comments').doc(commentId).get()
