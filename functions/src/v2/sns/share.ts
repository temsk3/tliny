...
export const v2_sns_share_shareContent = onCall(async (request) => {
  ...
  try {
    ...
    if (contentType === 'event') {
      ...
      const productDoc = await db.collection('v/1/products').doc(contentId).get()
    ...
    const shareData = {
      ...
      contentTitle: contentData?.name || contentData?.title || 'Unknown Content',
    ...
    if (contentType === 'post') {
      await db.collection('v/1/posts').doc(contentId).update({
    ...
    V2Logger.success(methodName, { shareId: shareRef.id, userId, contentId, platform })
    return createdShare
...
export const v2_sns_share_generateShareUrl = onCall(async (request) => {
  ...
  try {
    ...
    const description = contentData?.description || contentData?.content || 'Check this out on TLINY!'
...
export const v2_sns_share_getUserShares = onCall(async (request) => {
  ...
  try {
    ...
    V2Logger.success(methodName, { userId, targetUserId: queryUserId, count: shares.length })
    return { shares }
