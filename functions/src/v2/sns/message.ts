...
export const v2_sns_message_sendMessage = onCall(async (request) => {
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      // 会話の存在をチェック
      const conversationRef = db.collection('v/1/conversations').doc(conversationId)
      ...
      const messageData = {
        ...
        receiverId,
        receiverName: receiverData?.displayName || receiverData?.name || 'Unknown User',
    ...
    V2Logger.success(methodName, { messageId: messageRef.id, senderId, receiverId })
...
export const v2_sns_message_getMessages = onCall(async (request) => {
  ...
  try {
    ...
    // 会話の参加者であることを確認
    const conversationDoc = await db.collection('v/1/conversations').doc(conversationId).get()
    ...
    if (lastMessageId) {
      const lastMessageDoc = await db.collection('v/1/direct_messages').doc(lastMessageId).get()
