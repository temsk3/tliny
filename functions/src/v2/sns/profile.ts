...
export const v2_sns_profile_getProfile = onCall(async (request) => {
  ...
  try {
    ...
    // プロファイル情報を取得
    const profileDoc = await db.collection('v/1/user_profiles').doc(userId).get()
...
export const v2_sns_profile_updateProfile = onCall(async (request) => {
  ...
  try {
    ...
    const { displayName, bio, profileImageUrl, coverImageUrl, isPrivate } = request.data
    ...
    // 更新後のプロファイルを取得
    const updatedProfile = await db.collection('v/1/user_profiles').doc(userId).get()
    const profile = {
      id: updatedProfile.id,
      ...updatedProfile.data(),
    }
    
    V2Logger.success(methodName, { userId, fieldsUpdated: Object.keys(updateData) })
    return { profile }
