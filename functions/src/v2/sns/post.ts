...
export const v2_sns_post_createPost = onCall(async (request) => {
  ...
  try {
    ...
    const { content, imageUrls, eventId, productId, type = 'text' } = request.data
    ...
    // ユーザープロフィールの投稿数を更新
    await db.collection('v/1/user_profiles').doc(userId).update({
...
export const v2_sns_post_getFeed = onCall(async (request) => {
  ...
  try {
    ...
    const followingUserIds = followingQuery.docs.map(doc => doc.data().followingId)
