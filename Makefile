...
# packages upgrade --major-versions
.PHONY: upgrade-major
upgrade-major:
...
.PHONY: run-dev
run-dev:
	fvm flutter run -d chrome --web-renderer html --dart-define=FLAVOR=${DEV_FLAVOR} --target lib/main.dart

# run dev (with Device Preview)
.PHONY: run-dev-preview
run-dev-preview:
	fvm flutter run -d chrome --web-renderer html --dart-define=FLAVOR=${DEV_FLAVOR} --dart-define=PREVIEW=true --target lib/main.dart
